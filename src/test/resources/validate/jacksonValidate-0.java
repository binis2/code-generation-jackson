...
public class TestImpl implements Test, Modifiable<Test.Modify>, Validatable {
    ...
    public void validate() {
        Validation.form(this.getClass(), e -> Validation.start(e, this.getClass(), "value", value).validate(NullValidator.class, "({field}) Value can't be null").validateWithMessages(RangeValidator.class, new String[] { "min %2$d(%3$d)", "max %2$d(%4$d)" }, 0, 1000).validateWithMessages(LengthValidator.class, new String[] { "Value ({value}) for field '{field}' is shorter than {param[0]}!", "Value ({value}) for field '{field}' is longer than {param[1]}!" }, 1, 255).perform(v -> value = v), e -> Validation.start(e, this.getClass(), "sanitization", sanitization).sanitize(OnlyNotNullsLambdaSanitizer.class, ((java.util.function.Function<String, String>) String::toLowerCase)).sanitize(LengthSanitizer.class, 50).sanitize(TrimSanitizer.class).perform(v -> sanitization = v), e -> Validation.start(e, this.getClass(), "list", list).validate(NullValidator.class, "({field}) Value can't be null").validateCollection(NullValidator.class, "({field}) Value can't be null").validateWithMessagesCollection(RangeValidator.class, new String[] { "minC %2$d(%3$d)", "maxC %2$d(%4$d)" }, 0, 1000).validateWithMessagesCollection(LengthValidator.class, new String[] { "Value ({value}) for field '{field}' is shorter than {param[0]}!", "Value ({value}) for field '{field}' is longer than {param[1]}!" }, 1, 255).perform(v -> list = v));
    }
    ...
    protected class TestModifyImpl extends BaseModifierImpl<Test.Modify, Test> implements Test.Modify {
        ...
        public Test.Modify executions(String executions) {
            Validation.start(this.getClass(), "executions", executions).execute(LambdaExecutor.class, "({field}) Invalid value!", ((java.util.function.Consumer<String>) value -> {
